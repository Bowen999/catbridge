...
def get_target(name, df):
    ...
    Returns:
        pandas Series or None: The row corresponding to the target or None if not found.
    """
    if name in df.index:
        target_row = df.loc[name]
    else:
        print('Target is not in the index of the DataFrame')
        target_row = None  # or any other suitable value
...
def repeat_aggregation_max(df, design):
    ...
    for base_col in base_cols:
        for idx in new_df.index:
            ...
            # Check if sub_df_idx is a Series, if yes, get the first element
            if isinstance(sub_df_idx, pd.Series):
                sub_df_idx = sub_df_idx.iloc[0]
...
def plot_network(data, target_index, num_nodes=20, save_path=None):
    ...
    nx.draw_networkx_labels(G, pos, font_size=8, font_color='brown')
...
def plot_result(result, score_column, log2FoldChange_column, Granger_column, save_path=None):
    ...
    result['size'] = np.abs(result[log2FoldChange_column]) * 100 
...
def yuanfang(): 
    ...
    # openai_api_key = getpass.getpass("Please enter your OpenAI API Key: ")
    openai.api_key = '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'
    ...
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a highly skilled programmer and can explain complex technical concepts"
            },
            {
                "role": "user",
                "content": q
