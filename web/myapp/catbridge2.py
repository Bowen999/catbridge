...
def plot_ts_clusters(result, processed_gene, palette_name='crest', save_fig=False):
    ...
    for name, group in grouped:
        ...
        if save_fig:
            ...
            plt.savefig(f"myapp/result/plot/ts_cluster_{name}.png")
        else:
            plt.show()
...
def plot_result(result, score_column, log2FoldChange_column, Granger_column, save_path=None):
    ...
    """
    result = result.rename(columns={'Granger': 'Granger Score'})
    result = result.rename(columns={'log2FoldChange': 'NormFoldChange'})
    result = result.rename(columns={'Score': 'CAT Score'})
